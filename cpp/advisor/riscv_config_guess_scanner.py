...
class RiscvConfigGuessScanner(ConfigGuessScanner):

    """
    Scanner that scans config.guess files for riscv64 support.
    """

    TARGET_ARCH_RE = re.compile(r'(riscv64|riscv).*:Linux')
    ...
    def scan_file_object(self, filename, file_obj, report):
        ...
        for line in _lines:
            match = RiscvConfigGuessScanner.TARGET_ARCH_RE.search(line)
        ...
        # no match found.
            ...
            report.add_issue(ConfigGuessIssue(filename=filename,
                                              remark="autoconf config.guess needs updating to recognize riscv64 architecture"))
