...
from common.arch_strings import RISCV_ARCHS, NON_RISCV_ARCHS
...
from .arch_specific_libs import RISCV64_ARCH_SPECIFIC_LIBS
...
class RiscvMakefileScanner(MakefileScanner):

    """
    Scanner that scans Makefiles.
    """
    RISCV64_COMPILER_OPTION_CHECKPOINTS = []

    ARCH_SPECIFIC_LIBS_RE = re.compile(r'(%s)' % '|'.join([(r'%s\b' % x) for x in RISCV64_ARCH_SPECIFIC_LIBS]))

    #  ('!IF "$(VSCMD_ARG_TGT_ARCH)" == "riscv64"')
    RISCV64_CPU_LINE_RE = re.compile(r'\$\(VSCMD_ARG_TGT_ARCH\).*(%s)' % '|'.join(RISCV_ARCHS))

    OTHER_ARCH_CPU_LINE_RE = re.compile(r'\$\((?:CPU|PROCESSOR_ARCHITECTURE)\).*(%s)' % '|'.join(NON_RISCV_ARCHS))

    D_RISCV64_RE = re.compile(r'[-/]D(%s)' %
                              '|'.join(RISCV_ARCHS +
                                       [x.upper() for x in RISCV_ARCHS] +
                                       [('_%s_' % x) for x in RISCV_ARCHS] +
                                       [('_%s_' % x.upper()) for x in RISCV_ARCHS] +
                                       [('__%s__' % x) for x in RISCV_ARCHS] +
                                       [('__%s__' % x.upper()) for x in RISCV_ARCHS]))

    D_OTHER_ARCH_RE = re.compile(r'[-/]D(%s)' %
                                 '|'.join(NON_RISCV_ARCHS +
                                          [x.upper() for x in NON_RISCV_ARCHS] +
                                          [('_%s_' % x) for x in NON_RISCV_ARCHS] +
                                          [('_%s_' % x.upper()) for x in NON_RISCV_ARCHS] +
                                          [('__%s__' % x) for x in NON_RISCV_ARCHS] +
                                          [('__%s__' % x.upper()) for x in NON_RISCV_ARCHS]))
    ...
    def load_checkpoints(self):
        ...
        self.RISCV64_COMPILER_OPTION_CHECKPOINTS = init_checkpoints(
            self.checkpoints_content["RISCV64_COMPILER_OPTION_CHECKPOINTS"]
            )
    ...
    def scan_file_object(self, filename, file_obj, report):
        ...
        for lineno, line in enumerate(_lines, 1):
            ...
            #  other check points
            for c in self.RISCV64_COMPILER_OPTION_CHECKPOINTS:
