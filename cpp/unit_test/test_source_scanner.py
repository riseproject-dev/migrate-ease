...
class TestSourceScanner(unittest.TestCase):
    def setUp(self):
        self.source_scanner = RiscvSourceScanner(output_format=ReportOutputFormat.HTML, march='rv64gc', compiler='gcc', warning_level='L1')
    ...
    def test_scan_file_object_riscv(self):
        ...
        # __GNUC__ is a valid macro defined by clang/gcc.
        report = Report('/root')
        ...
        # On a RISC-V target, Arm/NEON intrinsics are incompatible too and must
        # be flagged (unlike on an Arm target, where they are the destination).
        report = Report('/root')
        io_object = io.StringIO('    vaddq_f32 (a, b);')
        self.source_scanner.scan_file_object('test.c',
                                             io_object,
                                             report)
        self.assertEqual(len(report.issues), 1)
    ...
    def test_equivalent_intrinsic_function_outline(self):
        # On a RISC-V target the #if defined(__otherarch__) branch is dead, so
        # the otherarch intrinsic in it is suppressed; the live #else holds no
        # intrinsic. No issue is reported.
        Report.REPORT_ITEM = BaseReportItem
        Report.REPORT_ITEM.TYPES += CPP_REPORT_TYPES
        report = Report('/root')
        self.source_scanner.initialize_report(report)
    ...
    def test_equivalent_intrinsic_function_inline(self):
        # As above, but with the dispatch inside the function body.
        Report.REPORT_ITEM = BaseReportItem
        Report.REPORT_ITEM.TYPES += CPP_REPORT_TYPES
        report = Report('/root')
        self.source_scanner.initialize_report(report)
    ...
    def test_intrinsic_in_live_riscv_branch(self):
        # An incompatible (x86) intrinsic in the live #else (RISC-V) branch is
        # flagged; the otherarch branch is dead and suppressed.
        Report.REPORT_ITEM = BaseReportItem
        Report.REPORT_ITEM.TYPES += CPP_REPORT_TYPES
        report = Report('/root')
        self.source_scanner.initialize_report(report)
        io_object = io.StringIO('#if defined(__otherarch__)\nvoid func() {\nfoo\n}\n#else\nvoid func() {\n_mm256_set1_epi32(123);\n}\n#endif')
        self.source_scanner.scan_file_object('test.c',
                                             io_object,
                                             report)
        self.source_scanner.finalize_report(report)
